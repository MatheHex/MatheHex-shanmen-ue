...
状态：`FREEZE_AUDIT_IN_PROGRESS`，不是整体冻结或 F 阶段验收。初版 P28.0，2026-09-16；最近完整产品根验证为 [P28.25](../Report/Dev.D.UE.0.0.10.P28.25.r0_report.md)，于 2026-09-21 完成新根1430/0、旧根1330/0、双构建及最终覆盖核验（UTC）。P28.1—25的路由、携入身份、分层拒绝保持、终局续清理与物品/容器World证据见第4节；P28.25闭合空间包部分生成失败回滚后的原owner续清理。后续审计只更新有限清单，不因“继续”无限追加系统。
...
| 编号 | 状态 | 关闭条件 |
|---|---|---|
| FZ-1 全入口权威路由 | P28.1 修复 Ready/归属混用；P28.2 修复携入堆叠身份缺口；P28.23闭合普通拾取销毁拒绝回滚；P28.24闭合空间包逻辑回收已接受后的原投影续清理、竞争丢弃门及修订不重做；P28.25闭合部分生成失败回滚中的原World投影保持与同对象续清理；全部入口审计仍待完成 | 覆盖产品整备/拾取/快捷使用/库存使用/终局，标注 Shanmen cutover 及旧兼容分支；说明 Code A Runtime 可变投影如何受 durable 结果约束。不能以局部修复代替全部调用图证明 |
...
- 最近完整产品验证：[P28.25 Report](../Report/Dev.D.UE.0.0.10.P28.25.r0_report.md)、[Log](../Log/Dev.D.UE.0.0.10.P28.25.r0_log.md)：Recovery1新根1430/0、旧根1330/0、双构建原生0/0，精确6路径映射覆盖3个必跑组，2760个独立成功用例；首次实际Red及首修失败原件保留。阶段闭合空间包部分生成回滚的原owner保持和续清理，不是FZ-1/2关闭后的最终冻结验证。
