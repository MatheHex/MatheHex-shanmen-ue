...
状态：`FREEZE_AUDIT_IN_PROGRESS`，不是整体冻结或 F 阶段验收。初版 P28.0，2026-09-16；最近完整产品根验证为 [P28.5](../Report/Dev.D.UE.0.0.10.P28.5.r0_report.md)，于 2026-09-17 完成新根 1428/0、旧根 1330/0 及双构建核验。P28.1 路由修补、P28.2 携入身份保护、P28.3/4/5 分层清理拒绝保持见第 4 节。后续审计更新本文的有限清单，不因“继续”无限追加系统。
...
| FZ-2 Run 激活失败及最终释放 | P28.3/4修复GameMode释放与Manager技术回滚拒绝保持；P28.5修复GameMode拒绝未阻止Manager局部清理；上层成功回滚后的完成确认/重试及其余激活/终局调用仍待核对 | 将 TryActivateCombatRun 的早期回滚与统一 Release 的适用范围说明清楚；对可达失败证明 owner 保留或安全回滚，对前置约束已排除的分支记录推理；EndPlay 不可当已成功清理的证据 |
...
剩余FZ-2限定为上层与可达性：Manager.DeactivateProfileWorld仍是void，成功技术回滚后的World释放完成确认/重试、一般ActivatePreparedProfileWorld调用点、终局及强制EndPlay的次序尚未全部取证。P28.5仅证明局部拒绝保持，不证明所有清理均原子化，也不以友元绑定夹具替代正式M01激活链。后续先核对现有调用条件，再决定最小修复；不得据本段扩充通用恢复系统。
...
- 最近完整产品验证：[P28.5 Report](../Report/Dev.D.UE.0.0.10.P28.5.r0_report.md)、[Log](../Log/Dev.D.UE.0.0.10.P28.5.r0_log.md)：新根1428/0、旧根1330/0、双构建原生0/0，精确八路径映射覆盖84个必跑组；不是FZ-1/2关闭后的最终冻结验证。此前 [P28.4](../Report/Dev.D.UE.0.0.10.P28.4.r0_report.md) 的Manager下层回滚拒绝保持、[P28.3](../Report/Dev.D.UE.0.0.10.P28.3.r0_report.md) 的GameMode拒绝保持和 [P27.31](../Report/Dev.D.UE.0.0.10.P27.31.r0_report.md) 的结算重放证据保留。
