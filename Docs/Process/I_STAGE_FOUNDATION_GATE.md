...
> 历史范围说明（2026-09-16，P28.0）：本文是 0.0.9B 的 I 阶段记录，不作为 0.0.10 当前门禁。其附件信封、等待下一 Prompt、P 阶段不跑自动化等规则均不适用于当前流程。0.0.10 使用 [P 阶段基线](P_STAGE_BASELINE_0_0_10.md) 与 [底层索引](../Architecture/Dev.D.UE.0.0.10_FoundationClosure_Index.md)。以下原文保留，不代表执行其中的产品启动、打包或结构迁移计划已获授权。
