	...
	EShanmenItemTransactionError* OutError)
{
	...
	TMap<FGuid, const FShanmenItemTransactionReceipt*> FinalizesByRunId;
		...
		Candidate.ProcessedRequests)
	{
		...
		const bool bLegacyClaim = Receipt.Operation
			== EShanmenItemTransactionOperation::ClaimPreparedRun;
		const bool bAtomicStart = Receipt.Operation
			== EShanmenItemTransactionOperation::StartPreparedRun;
		if (!bLegacyClaim && !bAtomicStart)
		{
			continue;
		}
		if (bLegacyClaim)
		...
		if (Receipt.ItemInstanceId != Receipt.RequestId && bAtomicStart)
		{
			return Fail();
		}
		if (ClaimsByRunId.Contains(Receipt.ReservationId)
		...
		Candidate.ProcessedRequests)
	{
		...
		if (!Claim || !*Claim
			...
			|| Receipt.AuthorityRevision <= (*Claim)->AuthorityRevision
			|| FinalizedRunIds.Contains(Receipt.ReservationId))
		{
			return Fail();
		}
		FinalizedRunIds.Add(Receipt.ReservationId);
		FinalizesByRunId.Add(Receipt.ReservationId, &Receipt);
	}
	...
	// Triggered defense resources may commit only while their exact prepared Run
	// is active. Every line must name a post-claim pending Durability/Charges
	// reservation for an item deployed by that claim.
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		Candidate.ProcessedRequests)
	{
		const FShanmenItemTransactionReceipt& Commit = Pair.Value.Receipt;
		if (!Commit.IsSuccess()
			|| Commit.Operation
		...
		const FShanmenItemTransactionReceipt* const* Claim =
			ClaimsByRunId.Find(Commit.ReservationId);
		const FShanmenItemTransactionReceipt* const* Finalize =
			FinalizesByRunId.Find(Commit.ReservationId);
		if (!Claim || !*Claim
			|| Commit.AuthorityRevision <= (*Claim)->AuthorityRevision
			|| (Finalize && *Finalize
				&& Commit.AuthorityRevision >= (*Finalize)->AuthorityRevision))
		{
			return Fail();
		}
		TMap<FGuid, FGuid> DeploymentByItem;
		for (const FGuid& PreparationReservationId :
			(*Claim)->ReservationIds)
		{
			const FShanmenItemReservationSnapshot* Preparation =
				Candidate.Reservations.Find(PreparationReservationId);
			if (Preparation && Preparation->ResourceKind
				== EShanmenItemResourceKind::DeploymentLock)
			{
				if (DeploymentByItem.Contains(Preparation->ItemInstanceId))
				{
					return Fail();
				}
				DeploymentByItem.Add(
					Preparation->ItemInstanceId,
					PreparationReservationId);
			}
		}
		for (const FGuid& ResourceReservationId : Commit.ReservationIds)
		{
			const FShanmenItemReservationSnapshot* ResourceReservation =
				Candidate.Reservations.Find(ResourceReservationId);
			const FShanmenItemProcessedRequestSnapshot* ReserveCommand =
				ResourceReservation
					? Candidate.ProcessedRequests.Find(
						ResourceReservation->ReserveRequestId)
					: nullptr;
			if (!ResourceReservation
				|| (ResourceReservation->ResourceKind
				...
				|| !DeploymentByItem.Contains(
					ResourceReservation->ItemInstanceId)
				|| !ReserveCommand || !ReserveCommand->Receipt.IsSuccess()
				|| ReserveCommand->Receipt.Operation
					!= EShanmenItemTransactionOperation::Reserve
				|| ReserveCommand->Receipt.AuthorityRevision
					<= (*Claim)->AuthorityRevision
				|| ReserveCommand->Receipt.AuthorityRevision
					>= Commit.AuthorityRevision)
			{
				return Fail();
			}
		}
	...
	// Resource-backed external impacts are a two-step durable saga. A prepare
	// receipt pins every reservation and freezes the triggered prefix without
	// changing resource totals. Exactly one later finalize receipt either commits
	// that prefix and cancels the remainder, or cancels the whole intent.
	TMap<FGuid, const FShanmenItemTransactionReceipt*>
		ResourceIntentPreparesByRequest;
	TSet<FGuid> ResourceIntentIds;
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		Candidate.ProcessedRequests)
	{
		const FShanmenItemTransactionReceipt& Prepare = Pair.Value.Receipt;
		...
		const FShanmenItemTransactionReceipt* const* Claim =
			ClaimsByRunId.Find(Prepare.ItemInstanceId);
		const FShanmenItemTransactionReceipt* const* RunFinalize =
			FinalizesByRunId.Find(Prepare.ItemInstanceId);
		if (!Claim || !*Claim || Pair.Key != Prepare.RequestId
			|| ResourceIntentPreparesByRequest.Contains(Prepare.RequestId)
			|| ResourceIntentIds.Contains(Prepare.ReservationId)
			|| Prepare.AuthorityRevision <= (*Claim)->AuthorityRevision
			|| (RunFinalize && *RunFinalize
				&& Prepare.AuthorityRevision >= (*RunFinalize)->AuthorityRevision))
		{
			return Fail();
		}
		TSet<FGuid> DeployedItems;
		for (const FGuid& DeploymentReservationId : (*Claim)->ReservationIds)
		{
			const FShanmenItemReservationSnapshot* Deployment =
				Candidate.Reservations.Find(DeploymentReservationId);
			if (Deployment && Deployment->ResourceKind
				== EShanmenItemResourceKind::DeploymentLock)
			{
				DeployedItems.Add(Deployment->ItemInstanceId);
			}
		}
		for (const FGuid& ReservationId : Prepare.ReservationIds)
		{
			...
			if (!Reservation
				|| (Reservation->ResourceKind
					...
					&& Reservation->ResourceKind
						!= EShanmenItemResourceKind::Charges)
				|| Reservation->PurposeId != Prepare.PurposeId
				|| !DeployedItems.Contains(Reservation->ItemInstanceId)
				|| !ReserveCommand || !ReserveCommand->Receipt.IsSuccess()
				|| ReserveCommand->Receipt.Operation
					!= EShanmenItemTransactionOperation::Reserve
				|| ReserveCommand->Receipt.AuthorityRevision
		...
		ResourceIntentPreparesByRequest.Add(Prepare.RequestId, &Prepare);
		ResourceIntentIds.Add(Prepare.ReservationId);
	}

	TSet<FGuid> FinalizedResourceIntentPrepareIds;
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		Candidate.ProcessedRequests)
	{
		const FShanmenItemTransactionReceipt& Finalize = Pair.Value.Receipt;
		...
		const FShanmenItemTransactionReceipt* const* Prepare =
			ResourceIntentPreparesByRequest.Find(Finalize.ItemInstanceId);
		if (!Prepare || !*Prepare
			|| FinalizedResourceIntentPrepareIds.Contains(Finalize.ItemInstanceId)
			|| Finalize.ReservationId != (*Prepare)->ReservationId
			|| Finalize.ReservationIds != (*Prepare)->ReservationIds
			|| Finalize.Amount != (*Prepare)->Amount
			|| Finalize.PurposeId != (*Prepare)->PurposeId
			|| Finalize.AuthorityRevision <= (*Prepare)->AuthorityRevision)
		{
			return Fail();
		}
		const FShanmenItemTransactionReceipt* const* RunFinalize =
			FinalizesByRunId.Find((*Prepare)->ItemInstanceId);
		if (RunFinalize && *RunFinalize
			&& Finalize.AuthorityRevision >= (*RunFinalize)->AuthorityRevision)
		{
			return Fail();
		}
		for (int32 Index = 0; Index < Finalize.ReservationIds.Num(); ++Index)
		{
			const FShanmenItemReservationSnapshot* Reservation =
				Candidate.Reservations.Find(Finalize.ReservationIds[Index]);
			const bool bShouldCommit =
				Finalize.Phase == EShanmenItemTransactionPhase::Committed
				&& Index < Finalize.Amount;
			if (!Reservation
				|| Reservation->PurposeId != Finalize.PurposeId
				|| Reservation->State != (bShouldCommit
					? EShanmenItemReservationState::Committed
		...
		FinalizedResourceIntentPrepareIds.Add(Finalize.ItemInstanceId);
	}

	int32 PendingResourceIntentCount = 0;
	for (const TPair<FGuid, const FShanmenItemTransactionReceipt*>& Pair :
		ResourceIntentPreparesByRequest)
	{
		if (FinalizedResourceIntentPrepareIds.Contains(Pair.Key))
		{
			continue;
		}
		++PendingResourceIntentCount;
		const FShanmenItemTransactionReceipt* Prepare = Pair.Value;
		if (!Prepare || FinalizedRunIds.Contains(Prepare->ItemInstanceId))
		{
			return Fail();
		}
		for (const FGuid& ReservationId : Prepare->ReservationIds)
	...
	if (PendingResourceIntentCount > 1)
	{
		return Fail();
	}

	// Active-Run Quantity actions use a separate two-step durable intent. The
	// prepare receipt freezes one exact stack balance without consuming it; one
	// later finalize receipt either appends the consumption or cancels it.
	TMap<FGuid, const FShanmenItemTransactionReceipt*>
		QuantityIntentPreparesByRequest;
	TMap<FGuid, const FShanmenItemTransactionReceipt*>
		QuantityIntentFinalizesByPrepare;
	TSet<FGuid> QuantityIntentIds;
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		Candidate.ProcessedRequests)
	{
		const FShanmenItemTransactionReceipt& Prepare = Pair.Value.Receipt;
		...
		const FGuid ActiveRunId = Prepare.ReservationIds.Num() == 1
			? Prepare.ReservationIds[0] : FGuid();
		const FShanmenItemTransactionReceipt* const* Claim =
			ClaimsByRunId.Find(ActiveRunId);
		const FShanmenItemTransactionReceipt* const* RunFinalize =
			FinalizesByRunId.Find(ActiveRunId);
		if (!Claim || !*Claim || Pair.Key != Prepare.RequestId
			|| QuantityIntentPreparesByRequest.Contains(Prepare.RequestId)
			|| QuantityIntentIds.Contains(Prepare.ReservationId)
			|| Prepare.AuthorityRevision <= (*Claim)->AuthorityRevision
			|| (RunFinalize && *RunFinalize
				&& Prepare.AuthorityRevision >= (*RunFinalize)->AuthorityRevision))
		{
		...
		int32 MatchingQuantityReservations = 0;
		for (const FGuid& ReservationId : (*Claim)->ReservationIds)
		{
			const FShanmenItemReservationSnapshot* Reservation =
				Candidate.Reservations.Find(ReservationId);
			const bool bReservationStateValid = Reservation
				&& (Reservation->State
						== EShanmenItemReservationState::Committed
					|| (RunFinalize && *RunFinalize
						&& Reservation->State
							== EShanmenItemReservationState::Released));
			MatchingQuantityReservations += Reservation
				&& Reservation->ItemInstanceId == Prepare.ItemInstanceId
				&& Reservation->ResourceKind
					== EShanmenItemResourceKind::Quantity
				&& bReservationStateValid ? 1 : 0;
		}
		if (MatchingQuantityReservations != 1)
		{
			return Fail();
		}
		QuantityIntentPreparesByRequest.Add(Prepare.RequestId, &Prepare);
		QuantityIntentIds.Add(Prepare.ReservationId);
	}
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		Candidate.ProcessedRequests)
	{
		...
		const FGuid PrepareRequestId = Finalize.ReservationIds.Num() == 2
			? Finalize.ReservationIds[1] : FGuid();
		const FShanmenItemTransactionReceipt* const* Prepare =
			QuantityIntentPreparesByRequest.Find(PrepareRequestId);
		if (!Prepare || !*Prepare
			|| QuantityIntentFinalizesByPrepare.Contains(PrepareRequestId)
			|| Finalize.ReservationId != (*Prepare)->ReservationId
			|| Finalize.ItemInstanceId != (*Prepare)->ItemInstanceId
			|| Finalize.ReservationIds[0] != (*Prepare)->ReservationIds[0]
			|| Finalize.Amount != (*Prepare)->Amount
			|| Finalize.PurposeId != (*Prepare)->PurposeId
			|| Finalize.AuthorityRevision <= (*Prepare)->AuthorityRevision)
		{
		...
		const FShanmenItemTransactionReceipt* const* RunFinalize =
			FinalizesByRunId.Find((*Prepare)->ReservationIds[0]);
		if (RunFinalize && *RunFinalize
			&& Finalize.AuthorityRevision >= (*RunFinalize)->AuthorityRevision)
		{
			return Fail();
		}
		QuantityIntentFinalizesByPrepare.Add(
			PrepareRequestId, &Finalize);
	}
		...
		QuantityIntentPreparesByRequest)
	{
		...
		const FGuid ActiveRunId = Prepare->ReservationIds[0];
		const FShanmenItemTransactionReceipt* const* Claim =
			ClaimsByRunId.Find(ActiveRunId);
		const FShanmenItemTransactionReceipt* const* Finalize =
			QuantityIntentFinalizesByPrepare.Find(Prepare->RequestId);
		...
		for (const FGuid& ReservationId : (*Claim)->ReservationIds)
		{
			...
			if (Reservation
				...
				&& Reservation->ResourceKind
					== EShanmenItemResourceKind::Quantity)
			{
				QuantityReservation = Reservation;
				break;
			}
		}
		...
		int32 ConsumedBeforePrepare = 0;
		int32 ConsumedBeforeFinalize = 0;
		const int32 FinalizeRevision = Finalize && *Finalize
			? (*Finalize)->AuthorityRevision : MAX_int32;
		for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& EventPair :
			Candidate.ProcessedRequests)
		{
			const FShanmenItemTransactionReceipt& Event =
				EventPair.Value.Receipt;
			if (!Event.IsSuccess())
			{
				continue;
			}
			const bool bDirectConsume = Event.Operation
					== EShanmenItemTransactionOperation::ConsumePreparedRunItem
				&& Event.ReservationId == ActiveRunId
				&& Event.ItemInstanceId == Prepare->ItemInstanceId;
			const bool bIntentCommit = Event.Operation
					== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
				&& Event.Phase == EShanmenItemTransactionPhase::Committed
				&& Event.ItemInstanceId == Prepare->ItemInstanceId
				&& Event.ReservationIds.Num() == 2
				&& Event.ReservationIds[0] == ActiveRunId;
			if (!bDirectConsume && !bIntentCommit)
			{
				continue;
			}
			if (Event.AuthorityRevision < Prepare->AuthorityRevision)
			{
				ConsumedBeforePrepare += Event.Amount;
			}
			if (Event.AuthorityRevision < FinalizeRevision)
			{
				ConsumedBeforeFinalize += Event.Amount;
			}
			if (bDirectConsume
				&& Event.AuthorityRevision > Prepare->AuthorityRevision
				&& Event.AuthorityRevision < FinalizeRevision)
			{
				return Fail();
			}
		}
		const int32 QuantityAtPrepare =
			QuantityReservation->Amount - ConsumedBeforePrepare;
		if (QuantityAtPrepare < Prepare->Amount
			|| Prepare->ResourceBefore != QuantityAtPrepare
			|| Prepare->ResourceAfter != QuantityAtPrepare
			|| Prepare->AvailableAfter
				!= QuantityAtPrepare - Prepare->Amount)
		{
			return Fail();
		}

		for (const TPair<FGuid, const FShanmenItemTransactionReceipt*>& OtherPair :
			QuantityIntentPreparesByRequest)
		{
			const FShanmenItemTransactionReceipt* Other = OtherPair.Value;
			if (!Other || Other == Prepare
				|| Other->ItemInstanceId != Prepare->ItemInstanceId
				|| Other->ReservationIds[0] != ActiveRunId)
			{
				continue;
			}
			if (Other->AuthorityRevision > Prepare->AuthorityRevision
				&& Other->AuthorityRevision < FinalizeRevision)
			{
				return Fail();
			}
		}
		...
		{
			const int32 QuantityAtFinalize =
				QuantityReservation->Amount - ConsumedBeforeFinalize;
			const bool bCommitted = (*Finalize)->Phase
				== EShanmenItemTransactionPhase::Committed;
			const int32 ExpectedAfter = bCommitted
				? QuantityAtFinalize - Prepare->Amount : QuantityAtFinalize;
			if (QuantityAtFinalize < Prepare->Amount
				|| (*Finalize)->ResourceBefore != QuantityAtFinalize
				|| (*Finalize)->ResourceAfter != ExpectedAfter
				|| (*Finalize)->AvailableAfter != ExpectedAfter)
			{
				return Fail();
			}
		}
		else if (FinalizedRunIds.Contains(ActiveRunId))
		{
			return Fail();
		}
	}

	// Prepared Quantity is no longer stored on the persistent tombstone after
	// StartPreparedRun. Reconstruct each active-Run balance exclusively from the
	// frozen reservation and this append-only receipt chain.
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		Candidate.ProcessedRequests)
	{
		const FShanmenItemTransactionReceipt& Consume = Pair.Value.Receipt;
		...
		const FShanmenItemTransactionReceipt* const* Claim =
			ClaimsByRunId.Find(Consume.ReservationId);
		const FShanmenItemTransactionReceipt* const* Finalize =
			FinalizesByRunId.Find(Consume.ReservationId);
		if (!Claim || !*Claim
			|| Consume.AuthorityRevision <= (*Claim)->AuthorityRevision
			|| (Finalize && *Finalize
				&& Consume.AuthorityRevision >= (*Finalize)->AuthorityRevision))
		{
			return Fail();
		}
		int32 MatchingQuantityReservations = 0;
		for (const FGuid& ReservationId : (*Claim)->ReservationIds)
		{
			const FShanmenItemReservationSnapshot* Reservation =
				Candidate.Reservations.Find(ReservationId);
			MatchingQuantityReservations += Reservation
				&& Reservation->ItemInstanceId == Consume.ItemInstanceId
				&& Reservation->ResourceKind
					== EShanmenItemResourceKind::Quantity ? 1 : 0;
		}
		if (MatchingQuantityReservations != 1)
		{
			return Fail();
	...
	for (const TPair<FGuid, const FShanmenItemTransactionReceipt*>& ClaimPair :
		ClaimsByRunId)
	{
		const FShanmenItemTransactionReceipt* Claim = ClaimPair.Value;
		if (!Claim)
		{
			return Fail();
		}
		for (const FGuid& ReservationId : Claim->ReservationIds)
		{
			const FShanmenItemReservationSnapshot* Reservation =
				Candidate.Reservations.Find(ReservationId);
			if (!Reservation || Reservation->ResourceKind
				!= EShanmenItemResourceKind::Quantity)
			{
				continue;
			}
			TArray<const FShanmenItemTransactionReceipt*> OrderedConsumes;
			for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
				Candidate.ProcessedRequests)
			{
				const FShanmenItemTransactionReceipt& Receipt = Pair.Value.Receipt;
				const bool bDirectConsume = Receipt.IsSuccess()
					&& Receipt.Operation
						== EShanmenItemTransactionOperation::ConsumePreparedRunItem
					&& Receipt.ReservationId == ClaimPair.Key
					&& Receipt.ItemInstanceId == Reservation->ItemInstanceId;
				const bool bIntentCommit = Receipt.IsSuccess()
					&& Receipt.Operation
						== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
					&& Receipt.Phase
						== EShanmenItemTransactionPhase::Committed
					...
					&& Receipt.ReservationIds[0] == ClaimPair.Key;
				if (bDirectConsume || bIntentCommit)
				{
					OrderedConsumes.Add(&Receipt);
				}
			}
			OrderedConsumes.Sort([](
			...
			int32 Remaining = Reservation->Amount;
			int32 PreviousAuthorityRevision = Claim->AuthorityRevision;
			for (const FShanmenItemTransactionReceipt* Consume : OrderedConsumes)
			{
				if (!Consume
					|| Consume->AuthorityRevision <= PreviousAuthorityRevision
					|| Consume->ResourceBefore != Remaining
					|| Consume->Amount > Remaining
					|| Consume->ResourceAfter != Remaining - Consume->Amount
					|| Consume->AvailableAfter != Consume->ResourceAfter)
				{
					return Fail();
				}
			...
			if (FinalizedRunIds.Contains(ClaimPair.Key))
			{
				...
				if (!Item || Item->Quantity > Remaining)
				{
					return Fail();
				}
			}
	...
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair : Candidate.ProcessedRequests)
	{
		...
		if (Processed.Receipt.IsSuccess())
		{
			...
			{
				for (int32 Index = 0;
					Index < Processed.Receipt.ReservationIds.Num(); ++Index)
				{
					const FGuid& ReservationId =
						Processed.Receipt.ReservationIds[Index];
					const bool bCommitted =
						Processed.Receipt.Phase
							== EShanmenItemTransactionPhase::Committed
						&& Index < Processed.Receipt.Amount;
					if (bCommitted)
					{
						if (CommittedReservationIds.Contains(ReservationId))
						{
							return Fail();
						}
					...
					{
						if (CancelledReservationIds.Contains(ReservationId))
						{
							return Fail();
						}
	...
	const FShanmenItemRunStartRequest& Request)
{
	...
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.StartPreparedRun.r1"), Parts);
}
...
{
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.ConsumePreparedRunItem.r1"),
		{
			GuidDigits(Request.Context.RunId),
			GuidDigits(Request.Context.OwnerId),
			GuidDigits(Request.Context.RequestId),
			...
			GuidDigits(Request.ItemInstanceId),
			FString::FromInt(Request.Amount),
			FString::FromInt(Request.ExpectedQuantityBefore),
			Request.PurposeId.ToString()
		});
}
...
{
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.PreparePreparedRunQuantityIntent.r1"),
		{
			GuidDigits(Request.Context.RunId),
			GuidDigits(Request.Context.OwnerId),
			GuidDigits(Request.Context.RequestId),
...
{
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.FinalizePreparedRunQuantityIntent.r1"),
		{
			GuidDigits(Request.Context.RunId),
			GuidDigits(Request.Context.OwnerId),
			GuidDigits(Request.Context.RequestId),
			...
			Request.Context.Content.Digest,
			GuidDigits(Request.ActiveRunId),
			GuidDigits(Request.PrepareRequestId),
			GuidDigits(Request.IntentId),
			GuidDigits(Request.ItemInstanceId),
			Request.bCommit ? TEXT("1") : TEXT("0")
		});
}
	...
	const FShanmenItemRunResourceCommitRequest& Request)
{
	TArray<FString> Parts =
	{
		...
		GuidDigits(Request.ActiveRunId),
		Request.PurposeId.ToString(),
		FString::FromInt(Request.OrderedLines.Num())
	};
	for (const FShanmenItemRunResourceCommitLine& Line :
		Request.OrderedLines)
	{
		Parts.Add(GuidDigits(Line.ReservationId));
		Parts.Add(GuidDigits(Line.ItemInstanceId));
	}
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.CommitPreparedRunResources.r1"), Parts);
}
	...
	const FShanmenItemRunResourceIntentRequest& Request)
{
	TArray<FString> Parts =
	{
		...
		GuidDigits(Request.ActiveRunId),
		GuidDigits(Request.IntentId),
		FString::FromInt(Request.TriggeredLineCount),
		Request.IntentMetadata.ToString(),
		FString::FromInt(Request.OrderedLines.Num())
	};
	for (const FShanmenItemRunResourceCommitLine& Line : Request.OrderedLines)
	{
		Parts.Add(GuidDigits(Line.ReservationId));
		Parts.Add(GuidDigits(Line.ItemInstanceId));
	}
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.PreparePreparedRunResourceIntent.r1"),
		Parts);
}
...
{
	return FShanmenDeterministicId::FromCanonicalParts(
		TEXT("Shanmen.Items.Command.FinalizePreparedRunResourceIntent.r1"),
		{
			GuidDigits(Request.Context.RunId),
			GuidDigits(Request.Context.OwnerId),
			GuidDigits(Request.Context.RequestId),
			...
			GuidDigits(Request.PrepareRequestId),
			GuidDigits(Request.IntentId),
			Request.bExternalCommitSucceeded ? TEXT("1") : TEXT("0")
		});
}
...
FShanmenItemTransactionReceipt FShanmenItemRepository::StartPreparedRun(
	const FShanmenItemRunStartRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
	if (bInitialized && Request.Context.RequestId.IsValid()
		&& TryReplay(
			Request.Context.RequestId, RequestFingerprint,
			EShanmenItemTransactionOperation::StartPreparedRun, Replayed))
	{
		return Replayed;
	}
	...
	FShanmenItemTransactionReceipt Receipt;
	Receipt.bSuccess = true;
	Receipt.Operation = EShanmenItemTransactionOperation::StartPreparedRun;
	Receipt.Phase = EShanmenItemTransactionPhase::Committed;
	Receipt.Error = EShanmenItemTransactionError::None;
	Receipt.RequestId = Request.Context.RequestId;
	Receipt.ReservationId = MakeActiveRunId(
		...
		Request.Context.RequestId);
	Receipt.ItemInstanceId = Request.Context.RequestId;
	Receipt.Amount = Request.ReservationIds.Num();
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
...
FShanmenItemTransactionReceipt FShanmenItemRepository::ConsumePreparedRunItem(
	const FShanmenItemRunConsumeRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
	...
	for (const FGuid& ReservationId : Claim->ReservationIds)
	{
		const FShanmenItemReservationSnapshot* Candidate =
			State.Reservations.Find(ReservationId);
		if (!Candidate
			|| Candidate->ItemInstanceId != Request.ItemInstanceId
	...
	const FShanmenItemInstance* Item =
		State.Items.Find(Request.ItemInstanceId);
	if (!QuantityReservation || !Item
		|| QuantityReservation->State
			!= EShanmenItemReservationState::Committed)
	{
		return Reject(EShanmenItemTransactionError::SecuredItemMismatch);
	}
	if (QuantityReservation->RunId != Request.Context.RunId
		|| QuantityReservation->OwnerId != Request.Context.OwnerId)
	{
		return Reject(EShanmenItemTransactionError::ScopeMismatch);
	}

	TSet<FGuid> FinalizedQuantityPrepareIds;
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		State.ProcessedRequests)
	{
		const FShanmenItemTransactionReceipt& Existing = Pair.Value.Receipt;
		if (Existing.IsSuccess()
			&& Existing.Operation
				== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
			&& Existing.ReservationIds.Num() == 2)
		{
			FinalizedQuantityPrepareIds.Add(Existing.ReservationIds[1]);
		}
	}
	int32 Consumed = 0;
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		State.ProcessedRequests)
	{
		...
		{
			Consumed += Existing.Amount;
		}
		else if (Existing.Operation
				== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
			&& Existing.Phase == EShanmenItemTransactionPhase::Committed
			&& Existing.ItemInstanceId == Request.ItemInstanceId
			&& Existing.ReservationIds.Num() == 2
			&& Existing.ReservationIds[0] == Request.ActiveRunId)
		{
			Consumed += Existing.Amount;
		}
		else if (Existing.Operation
				== EShanmenItemTransactionOperation::PreparePreparedRunQuantityIntent
			&& Existing.ItemInstanceId == Request.ItemInstanceId
			&& Existing.ReservationIds.Num() == 1
			&& Existing.ReservationIds[0] == Request.ActiveRunId
			&& !FinalizedQuantityPrepareIds.Contains(Existing.RequestId))
		{
			return Reject(
				EShanmenItemTransactionError::RunItemIntentConflict);
		}
	}
	const int32 QuantityBefore = QuantityReservation->Amount - Consumed;
	if (QuantityBefore < 0)
	{
		return Reject(EShanmenItemTransactionError::InvariantViolation);
	}
	if (Request.ExpectedQuantityBefore != QuantityBefore)
	{
		return Reject(EShanmenItemTransactionError::RunItemQuantityConflict);
	}
	if (Request.Amount > QuantityBefore)
	{
		return Reject(EShanmenItemTransactionError::InsufficientResource);
	}
	...
	Receipt.Amount = Request.Amount;
	Receipt.ResourceBefore = QuantityBefore;
	Receipt.ResourceAfter = QuantityBefore - Request.Amount;
	Receipt.AvailableAfter = Receipt.ResourceAfter;
	Receipt.ItemRevision = Item->Revision;
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
	Receipt.PurposeId = Request.PurposeId;
	Receipt.ReceiptId = MakeReceiptId(
...
FShanmenItemTransactionReceipt
FShanmenItemRepository::PreparePreparedRunQuantityIntent(
	const FShanmenItemRunQuantityIntentRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
		...
		State.ProcessedRequests)
	{
		...
		if (Existing.IsSuccess()
			...
			&& Existing.ReservationIds.Num() == 2)
		{
			FinalizedPrepareRequestIds.Add(Existing.ReservationIds[1]);
		}
	}
	int32 Consumed = 0;
	for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
		State.ProcessedRequests)
	{
			...
			&& Existing.ReservationIds[0] == Request.ActiveRunId)
		{
			...
			&& Existing.ReservationIds[0] == Request.ActiveRunId)
		{
			if (Existing.ReservationId == Request.IntentId
				|| !FinalizedPrepareRequestIds.Contains(Existing.RequestId))
			{
				return Reject(
					EShanmenItemTransactionError::RunItemIntentConflict);
			}
	...
	Receipt.Amount = Request.Amount;
	Receipt.ResourceBefore = QuantityBefore;
	Receipt.ResourceAfter = QuantityBefore;
	Receipt.AvailableAfter = QuantityBefore - Request.Amount;
	Receipt.ItemRevision = Item->Revision;
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
	Receipt.PurposeId = Request.PurposeId;
	Receipt.ReservationIds = { Request.ActiveRunId };
	Receipt.ReceiptId = MakeReceiptId(
		Request.Context.RequestId, RequestFingerprint,
		Receipt.Phase, Receipt.Error);
	RecordProcessed(
...
FShanmenItemTransactionReceipt
FShanmenItemRepository::FinalizePreparedRunQuantityIntent(
	const FShanmenItemRunQuantityIntentFinalizeRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
	...
	const FShanmenItemTransactionReceipt* Prepare = PrepareProcessed
		? &PrepareProcessed->Receipt : nullptr;
	if (!Prepare || !Prepare->IsSuccess()
		|| Prepare->Operation
			!= EShanmenItemTransactionOperation::PreparePreparedRunQuantityIntent
		|| Prepare->ReservationId != Request.IntentId
		|| Prepare->ItemInstanceId != Request.ItemInstanceId
		|| Prepare->ReservationIds.Num() != 1
		|| Prepare->ReservationIds[0] != Request.ActiveRunId)
	{
		return Reject(EShanmenItemTransactionError::RunItemIntentNotFound);
	}
		...
		State.ProcessedRequests)
	{
			...
			&& Existing.ItemInstanceId == Request.ItemInstanceId)
		{
			Consumed += Existing.Amount;
				...
				== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
			&& Existing.ReservationIds.Num() == 2
			&& Existing.ReservationIds[1] == Request.PrepareRequestId)
		{
			return Reject(
				EShanmenItemTransactionError::RunItemIntentConflict);
		}
	...
	const int32 QuantityBefore = QuantityReservation->Amount - Consumed;
	if (QuantityBefore < Prepare->Amount)
	{
		return Reject(EShanmenItemTransactionError::InvariantViolation);
	}
	...
	Receipt.ItemInstanceId = Request.ItemInstanceId;
	Receipt.ResourceKind = EShanmenItemResourceKind::Quantity;
	Receipt.Amount = Prepare->Amount;
	Receipt.ResourceBefore = QuantityBefore;
	Receipt.ResourceAfter = Request.bCommit
		? QuantityBefore - Prepare->Amount : QuantityBefore;
	Receipt.AvailableAfter = Receipt.ResourceAfter;
	Receipt.ItemRevision = Item->Revision;
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
	Receipt.PurposeId = Prepare->PurposeId;
	Receipt.ReservationIds =
		{ Request.ActiveRunId, Request.PrepareRequestId };
	Receipt.ReceiptId = MakeReceiptId(
		Request.Context.RequestId, RequestFingerprint,
		Receipt.Phase, Receipt.Error);
	RecordProcessed(
...
FShanmenItemTransactionReceipt
FShanmenItemRepository::CommitPreparedRunResources(
	const FShanmenItemRunResourceCommitRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
	...
	for (const FGuid& ReservationId : Claim->ReservationIds)
	{
		...
		if (Reservation && Reservation->ResourceKind
			== EShanmenItemResourceKind::DeploymentLock)
		{
			if (DeploymentByItem.Contains(Reservation->ItemInstanceId))
			{
				return Reject(
					EShanmenItemTransactionError::InvariantViolation);
			}
			DeploymentByItem.Add(
				Reservation->ItemInstanceId, ReservationId);
		}
	}
	...
	{
		const FShanmenItemReservationSnapshot* Reservation =
			State.Reservations.Find(Line.ReservationId);
		const FShanmenItemInstance* Item =
			State.Items.Find(Line.ItemInstanceId);
		const FGuid* DeploymentId =
			DeploymentByItem.Find(Line.ItemInstanceId);
		const FShanmenItemProcessedRequestSnapshot* ReserveCommand =
			Reservation
				? State.ProcessedRequests.Find(Reservation->ReserveRequestId)
				: nullptr;
		if (!Reservation)
		{
			return Reject(
		...
		if (!Item || Reservation->ItemInstanceId != Line.ItemInstanceId
			|| !DeploymentId
			|| Item->State != EShanmenItemInstanceState::Deployed
			|| Item->DeploymentReservationId != *DeploymentId)
		{
			return Reject(
				EShanmenItemTransactionError::SecuredItemMismatch);
		}
		if (Reservation->ResourceKind != EShanmenItemResourceKind::Durability
			&& Reservation->ResourceKind
				!= EShanmenItemResourceKind::Charges)
		{
			return Reject(
				EShanmenItemTransactionError::ResourceUnsupported);
		}
		if (Reservation->State
			!= EShanmenItemReservationState::Reserved)
		{
		...
		if (!ReserveCommand || !ReserveCommand->Receipt.IsSuccess()
			|| ReserveCommand->Receipt.Operation
				!= EShanmenItemTransactionOperation::Reserve
			|| ReserveCommand->Receipt.AuthorityRevision
				<= Claim->AuthorityRevision)
	...
	{
		EShanmenItemTransactionError CommitError;
		if (!ApplyCommit(Candidate, Line.ReservationId, CommitError))
		{
			return Reject(CommitError);
		}
	}
	...
	Receipt.ReservationId = Request.ActiveRunId;
	Receipt.ItemInstanceId = Request.Context.RequestId;
	Receipt.Amount = Request.OrderedLines.Num();
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
	Receipt.PurposeId = Request.PurposeId;
	for (const FShanmenItemRunResourceCommitLine& Line :
		Request.OrderedLines)
	{
		Receipt.ReservationIds.Add(Line.ReservationId);
	}
	Receipt.ReceiptId = MakeReceiptId(
		Request.Context.RequestId, RequestFingerprint,
		Receipt.Phase, Receipt.Error);
...
FShanmenItemTransactionReceipt
FShanmenItemRepository::PreparePreparedRunResourceIntent(
	const FShanmenItemRunResourceIntentRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
		...
		State.ProcessedRequests)
	{
			...
			&& Existing.ReservationId == Request.ActiveRunId)
		{
			...
			== EShanmenItemTransactionOperation::FinalizePreparedRunResourceIntent)
		{
			FinalizedPrepareRequestIds.Add(Existing.ItemInstanceId);
		}
	}
	if (!Claim)
	{
		...
		State.ProcessedRequests)
	{
		...
		if (Existing.IsSuccess()
			...
			&& !FinalizedPrepareRequestIds.Contains(Existing.RequestId))
		{
			return Reject(EShanmenItemTransactionError::ResourceIntentConflict);
		}
	}
	...
	for (const FShanmenItemRunResourceCommitLine& Line : Request.OrderedLines)
	{
		...
		if (Reservation->ResourceKind != EShanmenItemResourceKind::Durability
			&& Reservation->ResourceKind != EShanmenItemResourceKind::Charges)
		{
			return Reject(EShanmenItemTransactionError::ResourceUnsupported);
		}
		if (Reservation->State != EShanmenItemReservationState::Reserved)
	...
	{
		FShanmenItemReservationSnapshot* Reservation =
			Candidate.Reservations.Find(Line.ReservationId);
		if (!Reservation)
		{
			return Reject(EShanmenItemTransactionError::InvariantViolation);
		}
		Reservation->PurposeId = Request.IntentMetadata;
	}
	++Candidate.AuthorityRevision;
	...
	Receipt.RequestId = Request.Context.RequestId;
	Receipt.ReservationId = Request.IntentId;
	Receipt.ItemInstanceId = Request.ActiveRunId;
	Receipt.Amount = Request.TriggeredLineCount;
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
	Receipt.PurposeId = Request.IntentMetadata;
	for (const FShanmenItemRunResourceCommitLine& Line : Request.OrderedLines)
	{
		Receipt.ReservationIds.Add(Line.ReservationId);
	}
...
FShanmenItemTransactionReceipt
FShanmenItemRepository::FinalizePreparedRunResourceIntent(
	const FShanmenItemRunResourceIntentFinalizeRequest& Request)
{
	const FGuid RequestFingerprint = Fingerprint(Request);
	FShanmenItemTransactionReceipt Replayed;
		...
		State.ProcessedRequests)
	{
		...
		if (Existing.Operation
				== EShanmenItemTransactionOperation::PreparePreparedRunResourceIntent
			&& Existing.RequestId == Request.PrepareRequestId)
		{
			Prepare = &Existing;
		}
		else if (Existing.Operation
				== EShanmenItemTransactionOperation::FinalizePreparedRunResourceIntent
			&& Existing.ItemInstanceId == Request.PrepareRequestId)
		{
			return Reject(EShanmenItemTransactionError::ResourceIntentConflict);
		}
		else if (Existing.Operation
	...
	if (!Prepare || Prepare->ReservationId != Request.IntentId
		|| Prepare->ItemInstanceId != Request.ActiveRunId)
	{
		return Reject(EShanmenItemTransactionError::ResourceIntentNotFound);
	}
	if (bRunFinalized)
	{
		return Reject(EShanmenItemTransactionError::RunAlreadyFinalized);
	...
	FState Candidate = State;
	for (int32 Index = 0; Index < Prepare->ReservationIds.Num(); ++Index)
	{
		const FGuid& ReservationId = Prepare->ReservationIds[Index];
		if (Request.bExternalCommitSucceeded && Index < Prepare->Amount)
		{
			EShanmenItemTransactionError CommitError;
			if (!ApplyCommit(Candidate, ReservationId, CommitError))
			{
	...
	Receipt.Operation =
		EShanmenItemTransactionOperation::FinalizePreparedRunResourceIntent;
	Receipt.Phase = Request.bExternalCommitSucceeded
		? EShanmenItemTransactionPhase::Committed
		: EShanmenItemTransactionPhase::Cancelled;
	Receipt.Error = EShanmenItemTransactionError::None;
	Receipt.RequestId = Request.Context.RequestId;
	Receipt.ReservationId = Request.IntentId;
	Receipt.ItemInstanceId = Request.PrepareRequestId;
	Receipt.Amount = Prepare->Amount;
	Receipt.AuthorityRevision = Candidate.AuthorityRevision;
	Receipt.PurposeId = Prepare->PurposeId;
	Receipt.ReservationIds = Prepare->ReservationIds;
	Receipt.ReceiptId = MakeReceiptId(
		Request.Context.RequestId, RequestFingerprint,
		Receipt.Phase, Receipt.Error);
	RecordProcessed(
	...
	const FShanmenItemRunFinalizeRequest& Request)
{
		...
		State.ProcessedRequests)
	{
		...
		bool bIntentFinalized = false;
		for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& FinalizePair :
			State.ProcessedRequests)
		{
			const FShanmenItemTransactionReceipt& ExistingFinalize =
				FinalizePair.Value.Receipt;
			if (ExistingFinalize.IsSuccess()
				&& ExistingFinalize.Operation
					== EShanmenItemTransactionOperation::FinalizePreparedRunResourceIntent
				&& ExistingFinalize.ItemInstanceId == Prepare.RequestId)
			{
				bIntentFinalized = true;
				break;
			}
		}
		if (!bIntentFinalized)
		...
		State.ProcessedRequests)
	{
			...
			State.ProcessedRequests)
		{
			...
			if (ExistingFinalize.IsSuccess()
				&& ExistingFinalize.Operation
					== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
				&& ExistingFinalize.ReservationIds.Num() == 2
				&& ExistingFinalize.ReservationIds[1] == Prepare.RequestId)
			{
				bIntentFinalized = true;
				break;
			}
	...
	for (const FGuid& ReservationId : Claim->ReservationIds)
	{
		...
		int32 DurablyConsumed = 0;
		for (const TPair<FGuid, FShanmenItemProcessedRequestSnapshot>& Pair :
			State.ProcessedRequests)
		{
			const FShanmenItemTransactionReceipt& Existing = Pair.Value.Receipt;
			const bool bDirectConsume = Existing.IsSuccess()
				&& Existing.Operation
					== EShanmenItemTransactionOperation::ConsumePreparedRunItem
				&& Existing.ReservationId == Request.ActiveRunId
				&& Existing.ItemInstanceId == Item->ItemInstanceId;
			const bool bIntentCommit = Existing.IsSuccess()
				&& Existing.Operation
					== EShanmenItemTransactionOperation::FinalizePreparedRunQuantityIntent
				&& Existing.Phase == EShanmenItemTransactionPhase::Committed
				&& Existing.ItemInstanceId == Item->ItemInstanceId
			...
			if (bDirectConsume || bIntentCommit)
			{
				DurablyConsumed += Existing.Amount;
			}
		}
		const int32 MaximumSecuredQuantity =
			Reservation->Amount - DurablyConsumed;
		if (Reservation->ResourceKind != EShanmenItemResourceKind::Quantity
			|| MaximumSecuredQuantity < 0
			|| Remaining < 0 || Remaining > MaximumSecuredQuantity
			|| Item->State != EShanmenItemInstanceState::Depleted
			|| Item->Quantity != 0)
		{
			return Reject(
