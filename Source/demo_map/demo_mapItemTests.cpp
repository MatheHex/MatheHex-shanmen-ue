...
bool Fdemo_mapItemDefinitionsTest::RunTest(const FString&)
{
	...
	TestEqual(TEXT("Four stable slots"), Fdemo_mapItemDefinitions::GetEquipmentSlotIds().Num(), 4);
	...
	const Fdemo_mapItemDefinition* Accessory = Fdemo_mapItemDefinitions::Find(Fdemo_mapItemIds::WindTalisman);
	...
	const Fdemo_mapItemDefinition* EvasionAccessory = Fdemo_mapItemDefinitions::Find(Fdemo_mapItemIds::EvasionCharm);
	...
	TestTrue(TEXT("Required definitions exist"), Weapon && Armor && Accessory && Material && HeavyWeapon && ReinforcedArmor && EvasionAccessory && Iron && Token);
	if (!Weapon || !Armor || !Accessory || !Material || !HeavyWeapon || !ReinforcedArmor || !EvasionAccessory || !Iron || !Token) return false;
