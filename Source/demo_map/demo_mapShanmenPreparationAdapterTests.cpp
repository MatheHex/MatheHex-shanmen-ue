...
bool Fdemo_mapManagerRollbackContinuationTest::RunTest(const FString&)
{
	...
	for (const bool bViaGeneralStart : { false, true })
	{
		...
		// Match the immutable initialization route; ordinary Start must not take
		// ownership of an attempt retained by the framework coordinator.
		Manager->bUse0909BFrameworkHost = !bViaGeneralStart;
		Manager->bProfileWorldActive = true;
		Manager->bSettlementPending = true;
		...
		Weapon->SetRole(ROLE_SimulatedProxy);
		FString Diagnostic;
		const auto ContinueGeneralStart = [&]()
		{
			const auto Retry = Manager->StartPreparedProfileRunFromSect();
			TestTrue(TEXT("Ordinary continuation never reports a new active Run"),
				!Retry.IsRunActive() && Retry.Status == Edemo_mapProfileSessionBeginStatus::SessionNotReady);
			Diagnostic = Retry.Diagnostic;
			return !Manager->HasPendingProfileWorldRollback();
		};
		for (int32 Attempt = 0; Attempt < 2; ++Attempt)
		{
			TestFalse(TEXT("Technical rollback cannot acknowledge an unreleased World"),
				bViaGeneralStart
					? (Attempt == 0 ? Manager->RollbackPreparedProfileRunFor0909B(Diagnostic) : ContinueGeneralStart())
					: Attempt == 0
			...
			if (bViaGeneralStart)
			{
				TestTrue(TEXT("Ordinary retry retains only its own rollback, without a framework attempt"),
		...
		// Even a stale ordinary entry on a framework-owned Manager must not bypass
		// its coordinator. For the ordinary variant this also probes the route fence.
		Manager->bUse0909BFrameworkHost = true;
		...
		TestTrue(TEXT("Both start routes reject the pending rollback before input or new authority work"),
			...
			&& (bViaGeneralStart ? !Mode->Prepared0909BRunCorrelation.IsSet()
				: Mode->Prepared0909BRunCorrelation.IsSet()
					&& Mode->Prepared0909BRunCorrelation.GetValue() == Correlation));
		...
		if (bViaGeneralStart)
		{
			TestTrue(TEXT("Ordinary Start resumes the original pending World after fault removal"),
				ContinueGeneralStart());
			TestTrue(TEXT("Ordinary continuation reports completion without making a new Run"),
				Diagnostic.Contains(TEXT("rollback complete")) && !Mode->Prepared0909BRunCorrelation.IsSet());
		}
		else
		{
			TestFalse(TEXT("Ordinary route cannot steal a now-recoverable framework attempt"),
				Manager->StartPreparedProfileRunFromSect().IsRunActive());
			TestTrue(TEXT("Framework cleanup is still owned by its original upper attempt"),
				Manager->HasPendingProfileWorldRollback() && Coordinator.M01Adapter->IsAttemptPending(AttemptId)
				&& Mode->Prepared0909BRunCorrelation.IsSet());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	Fdemo_mapManagerSettlementContinuationTest,
	"Shanmen.0_0_10.Product.ControlledWeaponWorldLifecycle.ManagerSettlementContinuation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
