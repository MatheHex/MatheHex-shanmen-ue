...
try
{
    ...
    $ThrownArcPreviewPresentationCommandLedger = New-AutomationLogFixture `
        -Name 'thrown-arc-preview-presentation-command-ledger.log' `
        -Group 'Shanmen.0_0_10.Product.ThrownWeaponArcPreviewPresentationCommandLedger'
	...
	$ThrownArcPreviewPresentationOwnerSurfaceHandoff = New-AutomationLogFixture `
		-Name 'thrown-arc-preview-presentation-owner-surface-handoff.log' `
		-Group 'Shanmen.0_0_10.Product.ThrownWeaponArcPreviewPresentationOwnerSurfaceHandoff'
	$ThrownArcPreviewPresentationOwnerSurfaceHandoffRecovery = New-AutomationLogFixture `
		-Name 'thrown-arc-preview-presentation-owner-surface-handoff-recovery.log' `
		-Group 'Shanmen.0_0_10.Product.ThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecovery'
	...
	$ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundle = New-AutomationLogFixture `
		-Name 'thrown-arc-preview-presentation-owner-surface-handoff-recovery-bundle.log' `
		-Group 'Shanmen.0_0_10.Product.ThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundle'
	...
	$ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryTerminalAdoptionSession = New-AutomationLogFixture `
		-Name 'thrown-arc-preview-presentation-owner-surface-handoff-recovery-terminal-adoption-session.log' `
		-Group 'Shanmen.0_0_10.Product.ThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryTerminalAdoptionSession'
    ...
    $FormationScatterWorldPublication = New-AutomationLogFixture `
        -Name 'formation-scatter-world-publication.log' `
        -Group 'Shanmen.0_0_10.Product.FormationScatterWorldPublication'
    ...
    $FormationScatterWorldPublicationRunRoute = New-AutomationLogFixture `
        -Name 'formation-scatter-world-publication-run-route.log' `
        -Group 'Shanmen.0_0_10.Product.FormationScatterWorldPublicationRunRoute'
    ...
    Invoke-ExpectedPass `
        -Name 'formation scatter deployment commit requires resource chain item deployment core and full evidence' `
        -Paths @(
    ...
    Invoke-ExpectedPass `
        -Name 'formation scatter world publication requires port handoff deployment resource item and full evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenFormationScatterWorldPublication.h',
            'Source/demo_map/demo_mapShanmenFormationScatterWorldPublication.cpp') `
        -Logs @(
    ...
    Invoke-ExpectedPass `
        -Name 'formation scatter world publication session requires publication handoff deployment resource item and full evidence' `
        -Paths @(
    ...
    Invoke-ExpectedPass `
        -Name 'formation scatter world publication Run route requires Host session publication handoff deployment resource item and full evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenFormationScatterWorldPublicationRunRoute.h',
            'Source/demo_map/demo_mapShanmenFormationScatterWorldPublicationRunRoute.cpp') `
        -Logs @(
            $FormationScatterWorldPublicationRunRoute,
            $FormationScatterWorldPublicationCommandHost,
            $FormationScatterWorldPublicationSession,
            $FormationScatterWorldPublication,
            $FormationScatterWorldPlacementHandoff,
    ...
    Invoke-ExpectedPass `
        -Name 'controlled weapon active Run route is covered by the full suite' `
        -Paths @(
    ...
    Invoke-ExpectedPass `
        -Name 'sword qi world delivery maps contact through every authority seam' `
        -Paths @(
    ...
    Invoke-ExpectedPass `
        -Name 'sword qi command owner maps Run event identity through the input and product seams' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenSwordQiCommandEventOwner.cpp',
            'Source/demo_map/demo_mapShanmenSwordQiCommandEventOwnerTests.cpp') `
        -Logs @($Full, $Legacy)
	...
	Invoke-ExpectedPass `
		...
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcEditingInteractionComposition.h',
			'Source/demo_map/demo_mapShanmenThrownWeaponArcEditingInteractionComposition.cpp',
			'Source/demo_map/demo_mapShanmenThrownWeaponArcEditingInteractionCompositionTests.cpp') `
		-Logs @(
			$Full,
	...
	Invoke-ExpectedPass `
		...
		-Paths @(
			...
			'Source/demo_map/demo_mapShanmenThrownWeaponInputChoiceInteractionRequestCoordinatorTests.cpp') `
		-Logs @($Full)
	...
	Invoke-ExpectedPass `
		...
		-Paths @(
			...
			'Source/demo_map/demo_mapShanmenThrownWeaponInputChoiceIntentAdapterTests.cpp') `
		-Logs @($Full)
    ...
    Invoke-ExpectedPass `
		...
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponHotbarConfirmationAdapter.cpp',
			'Source/demo_map/demo_mapShanmenThrownWeaponHotbarConfirmationAdapterTests.cpp') `
		-Logs @($Full)

    Invoke-ExpectedPass `
		-Name 'thrown weapon Arc confirmation owner maps capture launch and broad evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcConfirmationOwner.cpp',
			'Source/demo_map/demo_mapShanmenThrownWeaponArcConfirmationOwnerTests.cpp') `
		-Logs @($Full)
    ...
    Invoke-ExpectedPass `
		...
		-Paths @(
			...
			'Source/demo_map/demo_mapShanmenThrownWeaponArcChoiceProjectionTests.cpp') `
		-Logs @($Full)

    Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview composition maps every delegated authority' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewComposition.h',
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewComposition.cpp',
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewCompositionTests.cpp') `
		-Logs @(
    ...
    Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview product bridge maps live read-only product and authority seams' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewProductBridge.h',
	...
	Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview Owner surface handoff recovery terminal adoption Session maps trusted completion and caller authority seams' `
		-Paths @(
	...
	Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview Owner surface handoff recovery completion Session maps durable completion and full authority seams' `
		-Paths @(
	...
	Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview Owner surface handoff recovery bundle storage maps bundle filesystem and full authority seams' `
		-Paths @(
	...
	Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview Owner surface handoff recovery bundle maps paired evidence and full authority seams' `
		-Paths @(
	...
	Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview Owner surface handoff recovery maps checkpoint ticket owner surface and full authority seams' `
		-Paths @(
	...
	Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview Owner surface handoff maps ticket lifecycle owner surface and full authority seams' `
		-Paths @(
	...
	Invoke-ExpectedPass `
		...
		-Paths @(
			...
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationDeliveryCoordinator.cpp') `
		-Logs @(
			$ThrownArcPreviewPresentationDeliveryCoordinator,
			$ThrownArcPreviewPresentationCommandLedger,
			$ThrownArcPreviewPresentationCommand,
    ...
    Invoke-ExpectedPass `
		-Name 'thrown weapon Arc preview presentation Command ledger maps receipt cursor and full authority seams' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationCommandLedger.h',
    ...
    Invoke-ExpectedPass `
        -Name 'armor resistance item adapter maps active Run projection item core and legacy armor evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenArmorResistanceItemAdapter.h',
            'Source/demo_map/demo_mapShanmenArmorResistanceItemAdapter.cpp',
            'Source/demo_map/demo_mapShanmenArmorResistanceItemAdapterTests.cpp') `
        -Logs @(
            $ArmorResistanceItemAdapter,
            $ArmorResistanceProjection,
            $Full,
            $CombatCore,
            $ItemUseAndArmor)
    ...
    Invoke-ExpectedPass `
        -Name 'Divine Sense product Controller maps frozen policy Session Run and every routed authority contract' `
        -Paths @(
    ...
    Invoke-ExpectedFail `
        -Name 'armor resistance item adapter focus cannot replace projection and shared authority evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenArmorResistanceItemAdapter.cpp') `
    ...
    Invoke-ExpectedFail `
        -Name 'Divine Sense product Controller focus cannot replace Session config and Run evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenDivineSenseProductController.cpp') `
        -Logs @(
            $DivineSenseProductController,
    ...
    Invoke-ExpectedFail `
        -Name 'sword qi world focus cannot replace coordinator world runtime and core evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenSwordQiWorldAdapter.cpp') `
    ...
    Invoke-ExpectedFail `
        -Name 'sword qi command-owner focus cannot replace adapter controller and legacy evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenSwordQiCommandEventOwner.cpp') `
    ...
    Invoke-ExpectedFail `
        -Name 'formation scatter deployment commit focus cannot replace resource chain item deployment and full evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenFormationScatterDeploymentCommit.cpp') `
    ...
    Invoke-ExpectedFail `
        -Name 'formation scatter world publication focus cannot replace port handoff deployment resource item and full evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenFormationScatterWorldPublication.cpp') `
        -Logs @($FormationScatterWorldPublication) `
        -ExpectedText 'missing required groups'

    Invoke-ExpectedFail `
        -Name 'formation scatter world publication session focus cannot replace publication handoff deployment resource item and full evidence' `
        -Paths @(
            'Source/demo_map/demo_mapShanmenFormationScatterWorldPublicationSession.cpp') `
    ...
    Invoke-ExpectedFail `
		-Name 'formation scatter world publication Run route focus cannot replace Host session deployment resource item and full evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenFormationScatterWorldPublicationRunRoute.cpp') `
		-Logs @($FormationScatterWorldPublicationRunRoute) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery next generation rotation focus cannot replace adopted completion and pending commit evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryNextGenerationRotationSession.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryNextGenerationRotationSession) `
		-ExpectedText 'missing required groups'

	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery terminal adoption focus cannot replace trusted completion and caller authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryTerminalAdoptionSession.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryTerminalAdoptionSession) `
		-ExpectedText 'missing required groups'

	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery completion focus cannot replace durable storage and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryCompletionSession.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryCompletionSession) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery bundle watermark focus cannot replace storage and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundleWatermarkAuthority.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundleWatermarkAuthority) `
		-ExpectedText 'missing required groups'

	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery bundle storage focus cannot replace bundle filesystem and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundleStorage.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundleStorage) `
		-ExpectedText 'missing required groups'

	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery bundle focus cannot replace paired codec and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundle.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryBundle) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown Arc editing interaction composition cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcEditingInteractionComposition.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown choice interaction request cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponInputChoiceInteractionRequestCoordinator.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown choice intent adapter cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponInputChoiceIntentAdapter.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
    ...
    Invoke-ExpectedFail `
		-Name 'thrown hotbar confirmation cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponHotbarConfirmationAdapter.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
    ...
    Invoke-ExpectedFail `
		-Name 'thrown Arc confirmation owner cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcConfirmationOwner.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
    ...
    Invoke-ExpectedFail `
		-Name 'thrown Arc source basis cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcSourceBasisAdapter.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
    ...
    Invoke-ExpectedFail `
		-Name 'thrown Arc choice projection cannot use unrelated item evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcChoiceProjection.cpp') `
		-Logs @($ItemUse) `
		-ExpectedText 'missing required groups'
    ...
    Invoke-ExpectedFail `
		-Name 'thrown Arc preview composition focus cannot replace delegated authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewComposition.cpp') `
    ...
    Invoke-ExpectedFail `
		-Name 'thrown Arc preview product bridge focus cannot replace live product and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewProductBridge.cpp') `
    ...
    Invoke-ExpectedFail `
		-Name 'thrown Arc preview presentation Command ledger focus cannot replace command state and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationCommandLedger.cpp') `
	...
	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery journal focus cannot replace checkpoint receipt and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecoveryJournal.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecoveryJournal) `
		-ExpectedText 'missing required groups'

	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner handoff recovery focus cannot replace ticket owner and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoffRecovery.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoffRecovery) `
		-ExpectedText 'missing required groups'

	Invoke-ExpectedFail `
		-Name 'thrown Arc preview Owner surface handoff focus cannot replace ticket lifecycle owner and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationOwnerSurfaceHandoff.cpp') `
		-Logs @($ThrownArcPreviewPresentationOwnerSurfaceHandoff) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown Arc preview surface recreation focus cannot replace owner surface and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationSurfaceRecreationPolicy.cpp') `
		-Logs @($ThrownArcPreviewPresentationSurfaceRecreationPolicy) `
		-ExpectedText 'missing required groups'
	...
	Invoke-ExpectedFail `
		-Name 'thrown Arc preview delivery coordinator focus cannot replace ledger command and authority evidence' `
		-Paths @(
			'Source/demo_map/demo_mapShanmenThrownWeaponArcPreviewPresentationDeliveryCoordinator.cpp') `
		-Logs @($ThrownArcPreviewPresentationDeliveryCoordinator) `
		-ExpectedText 'missing required groups'
