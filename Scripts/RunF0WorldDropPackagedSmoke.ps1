param(
    [string]$AttemptId = 'attempt-006',
    [string]$PackageAttemptId = 'attempt-004'
)
...
$RunRoot = Join-Path $TaskRoot "Smoke\WorldDrop-$AttemptId"
$ProductRoot = Join-Path $TaskRoot "ProductSmoke\WorldDrop-$AttemptId"
$PackageRoot = Join-Path $RunRoot 'Package'
$ManifestTool = Join-Path $PSScriptRoot 'GetF0CanonicalManifest.ps1'
$Utf8NoBom = [System.Text.UTF8Encoding]::new($false)

if (Test-Path -LiteralPath $RunRoot) { throw "World-drop smoke attempt already exists: $RunRoot" }
...
if ($Copy.ExitCode -gt 7) { throw "World-drop smoke copy failed: robocopy $($Copy.ExitCode)" }
...
if ($CopySummary.Files -ne $Clean.Files -or $CopySummary.Bytes -ne $Clean.Bytes -or $CopySummary.ManifestSha256 -ne $Clean.ManifestSha256) {
    throw 'World-drop smoke copy differs from clean Package.'
}

$CaseRoot = Join-Path $ProductRoot 'WorldDrop'
$UserRoot = Join-Path $CaseRoot 'UserDir'
$VisualRoot = Join-Path $CaseRoot 'Visual'
$LogPath = Join-Path $CaseRoot 'WorldDrop.log'
...
$Process = Start-Process -FilePath $Executable -ArgumentList @(
    ...
    '-ResX=1600','-ResY=900','-V3WorldUIVisibleAcceptance',
    "-V3WorldUIVisualOutput=$VisualRoot", "-UserDir=$UserRoot", "-abslog=$LogPath") `
...
$SuccessCount = [regex]::Matches($Text, 'V3_WORLD_UI_VISIBLE_ACCEPTANCE: PASS\.').Count
$FailureCount = [regex]::Matches($Text, 'V3_WORLD_UI_VISIBLE_ACCEPTANCE: FAIL').Count
...
$Result = [ordered]@{
    ...
    case_id = 'WorldDropAndRepick.Visible'
