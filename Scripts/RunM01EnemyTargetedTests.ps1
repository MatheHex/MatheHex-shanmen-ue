param([string]$TaskId = 'Manual.M01EnemyTargeted')
...
$Log = Join-Path $EvidenceDirectory 'M01EnemyTargetedTests.log'
...
& $EditorCmd $ProjectFile '/Game/M01/Maps/L_M01_Expedition' `
    ...
    '-ExecCmds=Automation RunTests demo_map.M01.Enemy;Quit' `
...
$Passed = @(Select-String -LiteralPath $Log -Pattern 'Test Completed\. Result=\{Success\}.*Path=\{demo_map\.M01\.Enemy\.' -AllMatches)
if ($Passed.Count -ne 3) { exit 1 }
